...
// File Name: edit.php
// Purpose: Allows event organizers to modify details of an existing event.
...
// 2. Fetch active values of the event
try {
...
// 3. Process form submission updates
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        try {
            $update_stmt = $db->prepare("UPDATE events SET title = :title, description = :description, date = :date, time = :time, location = :location, max_capacity = :max_capacity WHERE id = :id");
            $update_stmt->execute([
                'title' => $title,
            ...
            $_SESSION['success_message'] = "Event '$title' has been updated successfully!";
            header("Location: ../dashboard.php");
            exit();
        } catch (PDOException $e) {
...
$page_title = "Edit Event | Admin Portal";
...
<div class="admin-container">
    <div class="card-admin">
        <h2 class="admin-title"><i class="fa-solid fa-calendar-minus"></i> Edit Event Details</h2>
        <p class="admin-subtitle">Update schedule information, location, or maximum capacity constraints for this event.</p>
        ...
        <form action="edit.php?id=<?php echo $event_id; ?>" method="POST">
            ...
            <div class="form-group">
                ...
                <textarea id="description" name="description" class="form-control textarea-control" required><?php echo htmlspecialchars($event['description']); ?></textarea>
            </div>
            ...
            <div class="form-row">
                ...
                <div class="form-group">
                    ...
                    <input type="number" id="max_capacity" name="max_capacity" class="form-control" value="<?php echo intval($event['max_capacity']); ?>" required>
                </div>
            </div>

            <div class="action-group">
                <button type="submit" class="btn-admin-submit"><i class="fa-solid fa-floppy-disk"></i> Save Changes</button>
